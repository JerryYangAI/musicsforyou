...
// Admin middleware (legacy - for session-based auth)
  ...
  app.get("/api/music/result", async (req, res) => {
    try {
      ...
      if (result.status === "finished" && result.audioUrl) {
        try {
          // 先检查是否已存在（幂等性）
          let existingTrack = await trackStore.getTrackByTaskId(taskId);
            ...
            generateCoverArtForTrack(track)
              .then(async (coverUrl) => {
                if (coverUrl) {
                  ...
                  console.log(`[API] Track ${track.id} 封面已更新: ${coverUrl}`);
                }
              })
              .catch((err) => {
                console.error(`[API] Track ${track.id} 封面异步生成失败:`, err);
              });
          } else {
            console.log(`[API] Track 已存在，跳过保存: taskId=${taskId}`);
          }
        } catch (saveError) {
          // 保存失败不影响正常响应，只记录日志
  ...
  app.post("/api/prompt/optimize", async (req, res) => {
    try {
      ...
      res.status(500).json({
        ...
        message: error.message || "提示词优化失败，请稀后重试",
      });
    }
  });
